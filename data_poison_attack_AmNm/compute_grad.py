...
def compute_utility_grad(n_user, n_item, train, user_features_, item_features_, user_features_origin_,
...
def compute_grad(n_user, n_item, mal_user, mal_ratings, train, user_features_, mal_user_features_,
    ...
    for i in xrange(mal_user):
        for j in xrange(n_item):
            if j % 100 == 0:
                print('Computing the %dth malicious user, the %d item(total users: %d, total items: %d)' % (
                    i, j, n_user, n_item))
